...
use crate::persistence::repos::general::FindById;
use crate::types::Id;
    ...
    Conn::Transaction: FindById<File>,
{
    ...
    pub async fn execute(&self, id: Id, range: Option<Range>) -> Result<Chunk> {
        let file = self.find_file_in_db(id).await?;
        let range = range.unwrap_or_default();
        let chunk = Chunk::new(file, &range).await?;
        return Ok(chunk);
    }

    async fn find_file_in_db(&self, id: Id) -> Result<File> {
        self.conn
            .transaction()
            .await?
            .find_by_id(id)
